document.addEventListener("DOMContentLoaded", () => {
    ...
    if (loginForm) {
        loginForm.addEventListener("submit", function(event) {
            ...
            if (email === "") {
                emailError.textContent = "El campo de correo electrónico no puede estar vacío."; // Traducido
                return;
            }
            if (!isValidEmail(email)) {
                emailError.textContent = "Por favor, ingrese un formato de correo electrónico válido (ejemplo@dominio.com)."; // Traducido
                return;
            }
            emailError.textContent = "";
            window.location.href = "membros.html"; // Mantido, nome do arquivo
        });
    }
    ...
    // Lógica para a página de módulo (modulo.html)
    ...
    if (moduleTitleDisplay) { // Verifica se estamos na página do módulo
        const params = new URLSearchParams(window.location.search);
        const moduleType = params.get("type");
        const moduleName = decodeURIComponent(params.get("name") || "Módulo"); // Fallback, mas o nome já vem traduzido
        ...
        if (moduleVideosParam) {
            try {
                ...
                console.error("Error al analizar videos:", e); // Traducido
                moduleTitleDisplay.textContent = "Error al cargar datos."; // Traducido
                return;
            }
        }

        document.title = `${moduleName} - Área de Miembros`; // Traducido
        if(mainHeaderTitle) mainHeaderTitle.textContent = moduleName;
        ...
        if (moduleType === "ebook") {
            if (ebookContentDiv && moduleSrc) {
                ...
                ebookContentDiv.querySelector("iframe").src = moduleSrc;
                 ...
                 moduleTitleDisplay.textContent = "Contenido del Ebook no encontrado."; // Traducido
            }
            ...
            function loadVideo(index) {
                if (index >= 0 && index < moduleVideos.length) {
                    ...
                    if (videoId) {
                        videoPlayerEmbedDiv.innerHTML = `<iframe src="https://www.youtube.com/embed/${videoId}?autoplay=1&rel=0" title="${video.title}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
                    } else {
                        videoPlayerEmbedDiv.innerHTML = `<p>Error: URL del video no válida.</p>`; // Traducido
                    }
                    currentVideoTitleSpan.textContent = `${video.title} (Clase ${index + 1} de ${moduleVideos.length})`; // Traducido
            ...
            moduleTitleDisplay.textContent = "Contenido no encontrado o tipo inválido."; // Traducido
            ebookContentDiv.style.display = "none";
            videoAreaDiv.style.display = "none";
        }
    }
